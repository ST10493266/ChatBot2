...
namespace CybersecurityAwarenessBot.Service
{
    internal class ResponseEngine
    {
        ...
        private static readonly string[] _moreKeywords =
            new string[] { "another tip", "more", "tell me more", "explain more", "go on", "continue", "elaborate", "another one" };

        private static readonly string[] _confusionKeywords =
            new string[] { "don't understand", "confused", "what do you mean", "clarify", "can you explain", "huh", "unclear" };
        ...
        public string GetResponse(string input)
        {
            ...
            if (_moreKeywords.Any(k => trimmed.IndexOf(k, StringComparison.OrdinalIgnoreCase) >= 0))
            {
            ...
            if (_confusionKeywords.Any(k => trimmed.IndexOf(k, StringComparison.OrdinalIgnoreCase) >= 0))
            {
                if (_memory.CurrentTopic != null)
            ...
            foreach (var kvp in _randomResponses)
                ...
                {
                    _memory.CurrentTopic = kvp.Key;
                    return _memory.PersonalisedHint(kvp.Key) + GetTopicResponse(kvp.Key);
                }
            ...
            foreach (var kvp in _keywordResponses)
                ...
                {
                    _memory.CurrentTopic = kvp.Key;
                    return _memory.PersonalisedHint(kvp.Key) + kvp.Value;
                }
